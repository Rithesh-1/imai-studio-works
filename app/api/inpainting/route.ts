...
async function performInpainting(options: InpaintOptions): Promise<{ imageData: string }> {
  ...
  try {
    ...
    console.log(`📊 Main image data size: ${Math.round(imageData.length / 1024)}KB`);
    console.log(`📊 Mask data size: ${Math.round(maskData.length / 1024)}KB`);
...
IMPORTANT: You MUST call the image_generation tool to create the inpainted image. Do not respond with text only - you must generate an image.`;
    ...
    console.log('🚀 Step 4: Calling OpenAI API with responses...');
    console.log(`📤 Sending content with ${content.length} items to API...`);
    console.log(`📤 Content includes: 1 text prompt + 1 main image + ${referenceFileIds.length} reference images`);
    ...
    // Extract generated image data from the new API format
    ...
    if (generatedImageData.length > 0 && generatedImageData[0]) {
      ...
      console.error('❌ No image generation results found in response');
      console.error('❌ Response output types:', response.output.map(o => o.type));
  ...
  try {
    ...
    if (referenceImages && referenceImages.length > 0) {
      ...
      for (let i = 0; i < referenceImages.length; i++) {
        ...
        console.log(`✅ Reference image ${i + 1} validated`);
      }
    }

    // Check if OpenAI API key is available
    if (!process.env.OPENAI_API_KEY) {
      console.log("⚠️ OpenAI API key not found - returning mock response");
      return NextResponse.json({
        success: true,
    ...
    console.log("✅ Inpainting API completed successfully");
